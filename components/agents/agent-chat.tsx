...
export default function AgentChat({ agent, onClose }: AgentChatProps) {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      // Check if this is a Smart Money Analysis agent
      const isWalletAnalysisAgent = agent.description.toLowerCase().includes('smart money analysis');

      // Try wallet analysis processing first if it's a wallet analysis agent
      if (isWalletAnalysisAgent) {
        const walletAnalysisResult = await processMessageWithWalletAnalysis(
          messageContent,
          agent.id,
          true
        );

        if (walletAnalysisResult.shouldHandleLocally) {
          // Stop animation
          setIsAnimating(false);
          setCurrentStatus('');

          // Add wallet analysis response
          const agentMessageId = `${Date.now()}-agent`;
          const agentMessage: ChatMessage = {
            id: agentMessageId,
            content: walletAnalysisResult.response || 'Analysis completed',
            sender: "agent",
            timestamp: new Date(),
            agentId: agent.id,
          };

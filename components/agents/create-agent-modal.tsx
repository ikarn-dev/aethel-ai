...
interface FormData {
  name: string;
  strategy: string;
  tool: string; // Changed from tools array to single tool
  triggerType: string;
}
...
const AVAILABLE_TOOLS = [
  {
    ...
    value: 'llm_chat',
    label: 'LLM Chat',
    description: 'General purpose chat AI for conversations and assistance',
    config: {}
  },
  {
    id: 'llm_chat_wallet',
    ...
    description: 'AI-powered wallet analysis and trading insights using external data',
    config: {}
  }
];
...
export function CreateAgentModal({ isOpen, onClose, onSuccess, onShowToast }: CreateAgentModalProps) {
  ...
  const validateForm = (): boolean => {
    ...
    // Tool validation
    if (!formData.tool) {
      newErrors.tools = 'Please select a tool';
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Build tool configs - map UI tool ID to backend tool value
      const toolDef = AVAILABLE_TOOLS.find(t => t.id === formData.tool);
      const toolConfigs: ToolConfig[] = [{
        name: toolDef?.value || formData.tool, // Use the backend value (llm_chat)
      ...
      const agentConfig: AgentConfiguration = {
        ...
        description: `AI agent with ${toolDef?.label || 'AI'} capabilities`,
              ...
              {AVAILABLE_TOOLS.map(tool => (
                ...
                >
                  <input
                    type="radio"
                    name="tool-selection"
                    checked={formData.tool === tool.id}
                    onChange={() => handleToolSelect(tool.id)}
                    className="w-4 h-4 text-teal-400 bg-slate-800/50 border-teal-400/30 focus:ring-teal-400/50 focus:ring-1 cursor-pointer"
                    disabled={isSubmitting || isLoading}
                  />
